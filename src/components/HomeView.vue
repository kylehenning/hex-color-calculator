...
import { ref } from 'vue';
  ...
  const changeHex = () => {
    switch (operator.value) {
      case 'multiply':
        console.log("color 1", color1.value);
        console.log('color 2', color2.value)
        ...
        product = product % fakeMod;
        console.log("product: ", product)
        color3.value = product.toString(16);
    }
  ...
  const juliasWay = () => {
    ...
    console.log("here is final", final)
    color4.value = final;
...
<template>
  ...
  <div class="flex items-center m-10 flex-col">
    ...
    <div class="flex flex-row w-1/2 m-20 justify-between">
      ...
      <div :style="{'background-color': '#' + color3}" class=" w-1/4 border border-black" >PRODUCT</div>
    </div>
    <div class="flex flex-row w-1/2 m-20 justify-between">
      <button @click="juliasWay" class="bg-blue-400 w-24 p-5 rounded">JULIAS WAY</button>
      <div :style="{'background-color': '#' + color4}" class=" w-1/4 border border-black" >PRODUCT</div>
    </div>
  </div>
</template>
